...
public class ItextPrintUtil {
	...
	public void printPreview() {
		try {
			...
			MainFrame.showMessage(
					MainFrame.properties.getProperty("print.message"),
	...
	public File createHTML() {
		...
		try {
			...
			if (MainFrame.getInstance().getInterestCalculationPanel()
					.getAccountPanel().getTable().getCellEditor() != null)
				MainFrame.getInstance().getInterestCalculationPanel()
						.getAccountPanel().getTable().getCellEditor()
						.stopCellEditing();
			else
				System.out.println("cell editor null");

			// create account bean
			AccountBean account = MainFrame.getInstance().interestCalculationPanel
					.readForm();

			// validate account bean, if not valid print preview is not
			// possible!
				...
				MainFrame
						.showMessage(
								MainFrame.properties
										.getProperty("print.preview.account.invalid.from.date"),
								JOptionPane.ERROR_MESSAGE);
				return null;
			}
			case INVALID_TO_DATE: {
				MainFrame.showMessage(MainFrame.properties
						.getProperty("print.preview.account.invalid.to.date"),
						JOptionPane.ERROR_MESSAGE);
				return null;
			}
			case FROM_DATE_AFTER_TO_DATE: {
				MainFrame
						.showMessage(
								MainFrame.properties
										.getProperty("print.preview.account.invalid.from.date.after.to.date"),
								JOptionPane.ERROR_MESSAGE);
				return null;
			}
			case INVALID_START_AMOUNT: {
				MainFrame
						.showMessage(
								MainFrame.properties
										.getProperty("print.preview.account.invalid.start.amount"),
								JOptionPane.ERROR_MESSAGE);
				return null;
			}
			case INVALID_CREDITOR_DATA: {
				if (MainFrame.getInstance().configPanel.isUsingUserConfing()) {
					MainFrame
							.showMessage(
									MainFrame.properties
											.getProperty("print.preview.account.invalid.creditor.data"),
									JOptionPane.ERROR_MESSAGE);
					return null;
				}
				break;
			...
			case INVALID_DEBTOR_DATA: {
				if (MainFrame.getInstance().configPanel.isUsingUserConfing()) {
					MainFrame
							.showMessage(
									MainFrame.properties
											.getProperty("print.preview.account.invalid.debtor.data"),
									JOptionPane.ERROR_MESSAGE);
					return null;
				}
				break;
			...
			case INVALID_ACCOUNT_CHANGE_TABLE: {
				MainFrame
						.showMessage(
								MainFrame.properties
										.getProperty("print.preview.account.invalid.account.change.table"),
								JOptionPane.ERROR_MESSAGE);
				return null;
			}
			default: {
			...
			MainFrame.getInstance().getInterestCalculationPanel()
					.getAccountPanel().getNumberCellRenderer()
			...
			MainFrame
					.getInstance()
					.getInterestCalculationPanel()
					.getMainPanel()
					.getInterest()
					.setText(
							MainFrame.decimalFormater.format(account
									.getTotalInterest()));
			MainFrame
					.getInstance()
					.getInterestCalculationPanel()
			...
			String pageSizeName = MainFrame.printProperties
					.getProperty("document.page.size");
			ItextPrintUtil.getInstance().setPageSizeToDocument(document,
					pageSizeName);
			document.setMargins(Integer.parseInt(MainFrame.printProperties
					.getProperty("document.page.margin.left")), Integer
					.parseInt(MainFrame.printProperties
							.getProperty("document.page.margin.right")),
					Integer.parseInt(MainFrame.printProperties
							.getProperty("document.page.margin.top")),
					Integer.parseInt(MainFrame.printProperties
							.getProperty("document.page.margin.bottom")));

			writer.setBoxSize(
					"art",
					new Rectangle(
							Integer.parseInt(MainFrame.printProperties
									.getProperty("document.page.margin.left")),
							Integer.parseInt(MainFrame.printProperties
									.getProperty("document.page.margin.bottom")),
							Integer.parseInt(MainFrame.printProperties
									.getProperty("document.page.margin.left"))
			...
			// document title
			Paragraph documentTitle = createAndStyleParagraphLabel(
					"document.title", bf);
			documentTitle.setAlignment(Element.ALIGN_CENTER);
			documentTitle.setSpacingAfter(20);

			// print date
			Paragraph printDateLabel = createAndStyleParagraphLabel(
					"document.printDateLabel", bf);
			Paragraph printDate = createAndStyleParagraph(
					"document.printDateLabel",
					MainFrame.getInstance().sdfWithPoints.format(Calendar
							.getInstance().getTime()), bf);
			...
			PdfPCell creditorTableCell = createAndStyleTableCell(creditorTable,
					ResourceManager
							.getBaseColor("creditorTable.cell.background"),
					false);
			creditorTableCell.setBorder(Integer
					.parseInt(MainFrame.printProperties
							.getProperty("creditorTable.cell.border")));
			PdfPCell debtorTableCell = createAndStyleTableCell(
					debtorTable,
					ResourceManager.getBaseColor("debtorTable.cell.background"),
					false);
			debtorTableCell.setBorder(Integer
					.parseInt(MainFrame.printProperties
							.getProperty("debtorTable.cell.border")));
			...
			Paragraph creditorLabel = createAndStyleParagraphLabel(
					"creditor.label", bf);
			PdfPCell creditorLabelCell = createAndStyleTableCell(creditorLabel,
					ResourceManager
							.getBaseColor("creditor.label.cell.background"),
					true);
			creditorLabelCell.setBorder(Integer
					.parseInt(MainFrame.printProperties
							.getProperty("creditor.label.cell.border")));
			...
			innerCreditorTableCell.setBorder(Integer
					.parseInt(MainFrame.printProperties
							.getProperty("innerCreditorTable.cell.border")));
			...
			// debtor table cells
			Paragraph debtorLabel = createAndStyleParagraphLabel(
					"debtor.label", bf);
			PdfPCell debtorLabelCell = createAndStyleTableCell(debtorLabel,
					ResourceManager
							.getBaseColor("debtor.label.cell.background"), true);
			debtorLabelCell.setBorder(Integer
					.parseInt(MainFrame.printProperties
							.getProperty("debtor.label.cell.border")));
			...
			innerDebtorTableCell.setBorder(Integer
					.parseInt(MainFrame.printProperties
							.getProperty("innerDebtorTable.cell.border")));
			debtorTable.addCell(innerDebtorTableCell);
			// inner debtor table cells
			createDebtorInnerTables(bf, innerDebtorTable, account);
			String currency = MainFrame.printProperties
					.getProperty("document.calculatePeriod.label4");
			if (account.getInterestType() == AccountBean.ECB_INTEREST_TYPE
					|| account.getInterestType() == AccountBean.LAW_INTEREST_TYPE_EUR)
				currency = MainFrame.printProperties
						.getProperty("document.calculatePeriod.label4a");
			// calculate period
			String calculatePeriodLabel = new String(
					MainFrame.printProperties
							.getProperty("document.calculatePeriod.label1")
							+ MainFrame.sdfWithPoints.format(account.getFrom())
							+ " "
							+ MainFrame.printProperties
									.getProperty("document.calculatePeriod.label2")
							+ MainFrame.sdfWithPoints.format(account.getTo())
							+ " "
							+ MainFrame.printProperties
									.getProperty("document.calculatePeriod.label3")
							+ MainFrame.decimalFormater.format(account
									.getStartAmount()) + " " + currency);
			...
			Paragraph documentNumber = createAndStyleParagraph(
					...
					MainFrame.printProperties
							.getProperty("document.number.label")
							+ account.getDocument(), bf);
			...
			Paragraph appliedRate = createAndStyleParagraph(
					...
					MainFrame.printProperties
							.getProperty("document.appliedRate.label")
							+ getInterestTypeByName(account), bf);
			...
			Paragraph appliedMethod = createAndStyleParagraph(
					"document.appliedRate.label",
					MainFrame.printProperties
							.getProperty("document.appliedMethod.label")
							+ getMethodTypeByName(account), bf);
	...
	static class HeaderFooter extends PdfPageEventHelper {
		...
		public void onEndPage(PdfWriter writer, Document document) {
			...
			System.out.println("footer font size: "
					+ footerLabel.getFont().getCalculatedSize());
	...
	private Paragraph createAndStyleParagraphLabel(String label, BaseFont bf) {
		String paragraphString = new String(
				MainFrame.printProperties.getProperty(label));
		Font paragraphFont = new Font(bf,
				Integer.parseInt(MainFrame.printProperties.getProperty(label
						+ ".font.size")),
				Integer.parseInt(MainFrame.printProperties.getProperty(label
						+ ".font.style")), new BaseColor(
						Integer.parseInt(MainFrame.printProperties
								.getProperty(label + ".font.color"))));
	...
	private Paragraph createAndStyleParagraph(String label, String value,
			BaseFont bf) {
	...
	private PdfPCell createAndStyleTableCell(Element cellElement,
			BaseColor backgroundColor, boolean vAlign) {
	...
	public PdfPTable createCreditorInnerTables(BaseFont bf,
			PdfPTable innerTable, AccountBean account) {
		// name
		Paragraph creditorNameLabel = createAndStyleParagraph(
				"creditor.data.label",
				MainFrame.printProperties
						.getProperty("creditorAndDebtor.name.label"), bf);
		PdfPCell nameLabelCell = createAndStyleTableCell(creditorNameLabel,
				ResourceManager
						.getBaseColor("creditor.data.label.cell.background"),
				true);
		// fill with data from creditor bean
		Paragraph creditorName = createAndStyleParagraph("creditor.data",
				account.getCreditor().getName(), bf);
		PdfPCell nameCell = createAndStyleTableCell(creditorName,
				ResourceManager.getBaseColor("creditor.data.cell.background"),
				true);
		...
		Paragraph creditorAddressLabel = createAndStyleParagraph(
				"creditor.data.label",
				MainFrame.printProperties
						.getProperty("creditorAndDebtor.address.label"), bf);
		PdfPCell addressLabelCell = createAndStyleTableCell(
				creditorAddressLabel,
				ResourceManager
						.getBaseColor("creditor.data.label.cell.background"),
				true);
		// fill with data from creditor bean
		Paragraph creditorAddress = createAndStyleParagraph("creditor.data",
				account.getCreditor().getAddress(), bf);
		PdfPCell addressCell = createAndStyleTableCell(creditorAddress,
				ResourceManager.getBaseColor("creditor.data.cell.background"),
				true);
		...
		Paragraph creditorCityLabel = createAndStyleParagraph(
				"creditor.data.label",
				MainFrame.printProperties
						.getProperty("creditorAndDebtor.city.label"), bf);
		PdfPCell cityLabelCell = createAndStyleTableCell(creditorCityLabel,
				ResourceManager
						.getBaseColor("creditor.data.label.cell.background"),
				true);
		// fill with data from creditor bean
		Paragraph creditorCity = createAndStyleParagraph("creditor.data",
				account.getCreditor().getCity(), bf);
		PdfPCell cityCell = createAndStyleTableCell(creditorCity,
				ResourceManager.getBaseColor("creditor.data.cell.background"),
				true);
		...
		Paragraph creditorPibLabel = createAndStyleParagraph(
				"creditor.data.label",
				MainFrame.printProperties
						.getProperty("creditorAndDebtor.pib.label"), bf);
		PdfPCell pibLabelCell = createAndStyleTableCell(creditorPibLabel,
				ResourceManager
						.getBaseColor("creditor.data.label.cell.background"),
				true);
		// fill with data from creditor bean
		Paragraph creditorPib = createAndStyleParagraph("creditor.data",
				account.getCreditor().getPib(), bf);
		PdfPCell pibCell = createAndStyleTableCell(creditorPib,
				ResourceManager.getBaseColor("creditor.data.cell.background"),
				true);
		...
		Paragraph creditorPhoneLabel = createAndStyleParagraph(
				"creditor.data.label",
				MainFrame.printProperties
						.getProperty("creditorAndDebtor.phone.label"), bf);
		PdfPCell phoneLabelCell = createAndStyleTableCell(creditorPhoneLabel,
				ResourceManager
						.getBaseColor("creditor.data.label.cell.background"),
				true);
		// fill with data from creditor bean
		Paragraph creditorPhone = createAndStyleParagraph("creditor.data",
				account.getCreditor().getPhone(), bf);
		PdfPCell phoneCell = createAndStyleTableCell(creditorPhone,
				ResourceManager.getBaseColor("creditor.data.cell.background"),
				true);
	...
	public PdfPTable createDebtorInnerTables(BaseFont bf, PdfPTable innerTable,
			AccountBean account) {
		// name
		Paragraph debtorNameLabel = createAndStyleParagraph(
				...
				MainFrame.printProperties
						.getProperty("creditorAndDebtor.name.label"), bf);
		PdfPCell nameLabelCell = createAndStyleTableCell(debtorNameLabel,
				ResourceManager
						.getBaseColor("debtor.data.label.cell.background"),
				true);
		// fill with data from debtor bean
		Paragraph debtorName = createAndStyleParagraph("debtor.data", account
				.getDebtor().getName(), bf);
		PdfPCell nameCell = createAndStyleTableCell(debtorName,
				ResourceManager.getBaseColor("debtor.data.cell.background"),
				true);
		...
		Paragraph debtorAddressLabel = createAndStyleParagraph(
				...
				MainFrame.printProperties
						.getProperty("creditorAndDebtor.address.label"), bf);
		PdfPCell addressLabelCell = createAndStyleTableCell(debtorAddressLabel,
				ResourceManager
						.getBaseColor("debtor.data.label.cell.background"),
				true);
		// fill with data from debtor bean
		Paragraph debtorAddress = createAndStyleParagraph("debtor.data",
				account.getDebtor().getAddress(), bf);
		PdfPCell addressCell = createAndStyleTableCell(debtorAddress,
				ResourceManager.getBaseColor("debtor.data.cell.background"),
				true);
		...
		Paragraph debtorCityLabel = createAndStyleParagraph(
				...
				MainFrame.printProperties
						.getProperty("creditorAndDebtor.city.label"), bf);
		PdfPCell cityLabelCell = createAndStyleTableCell(debtorCityLabel,
				ResourceManager
						.getBaseColor("debtor.data.label.cell.background"),
				true);
		// fill with data from debtor bean
		Paragraph debtorCity = createAndStyleParagraph("debtor.data", account
				.getDebtor().getCity(), bf);
		PdfPCell cityCell = createAndStyleTableCell(debtorCity,
				ResourceManager.getBaseColor("debtor.data.cell.background"),
				true);

		// PIB
		Paragraph debtorPibLabel = createAndStyleParagraph("debtor.data.label",
				MainFrame.printProperties
						.getProperty("creditorAndDebtor.pib.label"), bf);
		PdfPCell pibLabelCell = createAndStyleTableCell(debtorPibLabel,
				ResourceManager
						.getBaseColor("debtor.data.label.cell.background"),
				true);
		// fill with data from debtor bean
		Paragraph debtorPib = createAndStyleParagraph("debtor.data", account
				.getDebtor().getPib(), bf);
		PdfPCell pibCell = createAndStyleTableCell(debtorPib,
				ResourceManager.getBaseColor("debtor.data.cell.background"),
				true);
		...
		Paragraph debtorPhoneLabel = createAndStyleParagraph(
				...
				MainFrame.printProperties
						.getProperty("creditorAndDebtor.phone.label"), bf);
		PdfPCell phoneLabelCell = createAndStyleTableCell(debtorPhoneLabel,
				ResourceManager
						.getBaseColor("debtor.data.label.cell.background"),
				true);
		// fill with data from debtor bean
		Paragraph debtorPhone = createAndStyleParagraph("debtor.data", account
				.getDebtor().getPhone(), bf);
		PdfPCell phoneCell = createAndStyleTableCell(debtorPhone,
				ResourceManager.getBaseColor("debtor.data.cell.background"),
				true);
	...
	public PdfPTable createAccountResultTable(BaseFont bf, AccountBean account) {
		...
		PdfPTable table = new PdfPTable(new float[] { 1, 1, 1.1f, 0.8f, 0.7f,
				1f, 1.1f, 1f, 1, 1.1f });
		...
		for (int i = 0; i < 1; i++) {
			Paragraph dateFromHeader = createAndStyleParagraph(
					...
					MainFrame.printProperties
							.getProperty("accountResultTable.dateFrom"), bf);
			...
			Paragraph dateToHeader = createAndStyleParagraph(
					"accountResultTable.data",
					MainFrame.printProperties
							.getProperty("accountResultTable.dateTo"), bf);
			...
			Paragraph amountHeader = createAndStyleParagraph(
					"accountResultTable.data",
					MainFrame.printProperties
							.getProperty("accountResultTable.amount"), bf);
			...
			Paragraph rateHeader = createAndStyleParagraph(
					"accountResultTable.data",
					MainFrame.printProperties
							.getProperty("accountResultTable.rate"), bf);
			...
			Paragraph methodHeader = createAndStyleParagraph(
					"accountResultTable.data",
					MainFrame.printProperties
							.getProperty("accountResultTable.method"), bf);
			...
			Paragraph interestHeader = createAndStyleParagraph(
					"accountResultTable.data",
					MainFrame.printProperties
							.getProperty("accountResultTable.interest"), bf);
			...
			Paragraph interestChangeHeader = createAndStyleParagraph(
					"accountResultTable.data",
					MainFrame.printProperties
							.getProperty("accountResultTable.interestChange"),
					bf);
			...
			Paragraph callToHeader = createAndStyleParagraph(
					"accountResultTable.data",
					MainFrame.printProperties
							.getProperty("accountResultTable.callTo"), bf);
			...
			Paragraph dateOfChangeHeader = createAndStyleParagraph(
					"accountResultTable.data",
					MainFrame.printProperties
							.getProperty("accountResultTable.dateOfChange"), bf);
			...
			Paragraph debtHeader = createAndStyleParagraph(
					"accountResultTable.data",
					MainFrame.printProperties
							.getProperty("accountResultTable.debt"), bf);
		...
		for (AccountResultBean resultBean : account.getResults()) {
			...
			if (MainFrame.getInstance().sdfWithPoints.format(resultBean
					.getFrom()) != null) {
				Paragraph dateFrom = createAndStyleParagraph(
						"accountResultTable.data",
						MainFrame.getInstance().sdfWithPoints.format(resultBean
								.getFrom()), bf);
			...
			if (MainFrame.getInstance().sdfWithPoints
					.format(resultBean.getTo()) != null) {
			...
			Paragraph amount = createAndStyleParagraph(
					"accountResultTable.data",
					MainFrame.decimalFormater.format(resultBean.getAmount()),
					bf);
			...
			Paragraph interestRate = createAndStyleParagraph(
					"accountResultTable.data",
					MainFrame.rateFormater.format(resultBean.getInterestRate()),
					bf);
			...
			Paragraph interest = createAndStyleParagraph(
					"accountResultTable.data",
					MainFrame.decimalFormater.format(resultBean.getInterest()),
					bf);
			...
			Paragraph change = createAndStyleParagraph(
					"accountResultTable.data",
					MainFrame.decimalFormater.format(resultBean.getChange()),
					bf);
			...
			if (resultBean.getDocument() != null) {
				Paragraph document = createAndStyleParagraph(
						"accountResultTable.data", resultBean.getDocument(), bf);
			...
			Paragraph debt = createAndStyleParagraph("accountResultTable.data",
		...
		Paragraph totalHeader = createAndStyleParagraph(
				...
				MainFrame.printProperties
						.getProperty("accountResultTable.total"), bf);
	...
	private String methodToString(double method) {
		...
		if (method == AccountBean.CONFORM_METHOD_TYPE)
			return MainFrame.properties.getProperty("conform.method.label");
		if (method == AccountBean.PROPORCIONAL_METHOD_TYPE)
			return MainFrame.properties
					.getProperty("proporcional.method.label");
		return "";
	}
	...
	public void styleHeaderCellAccountResultTable(PdfPCell cell) {
		...
		cell.setBackgroundColor(new BaseColor(Integer
				.parseInt(MainFrame.printProperties
						.getProperty("accountResultTable.header.background"))));
	}
